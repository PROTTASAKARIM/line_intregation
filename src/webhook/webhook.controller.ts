    ...
    if (events && events.length > 0) {
      ...
      for (const event of events) {
        ...
        if (event.type === 'postback') {
          ...
          if (action === 'buy') {
            await this.sendReply(
              event.replyToken,
              `You chose to buy item with ID: ${itemId}`,
            );
          }
        }
        if (event.type === 'beacon') {
          ...
          console.log(
            `Beacon event: ${beaconType}, Beacon ID: ${beaconId}, Device ID: ${deviceId}`,
          );
          ...
          if (beaconType === 'enter') {
            await this.sendReply(
              event.replyToken,
              `You are now near Beacon ID: ${beaconId}`,
            );
          } else if (beaconType === 'leave') {
            await this.sendReply(
              event.replyToken,
              `You have left the range of Beacon ID: ${beaconId}`,
            );
          }
        }
        if (event.type === 'accountLink') {
          ...
          console.log(
            `Account Link event: ${linkStatus} for User ID: ${userId}`,
          );
